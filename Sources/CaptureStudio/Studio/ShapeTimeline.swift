...
/// Pure block math for the shape overlay track: which blocks are visible at a
/// time, the add / move / remove operations the lane UI drives, and z-order
/// reordering + greedy sub-row packing for the lane layout. No AVFoundation,
/// no UI — all unit-tested.
///
/// This is the multi-instance, overlap-allowed counterpart to `CameraTimeline`,
/// identical in shape to `TextTimeline`:
...
enum ShapeTimeline {
    ...
    static func active(at t: Double, blocks: [ShapeBlock]) -> [ShapeBlock] {
    ...
    static func add(_ blocks: [ShapeBlock], atTime: Double, width: Double,
                    duration: Double, template: ShapeBlock)
        -> (blocks: [ShapeBlock], id: UUID) {
    ...
    static func moveBegin(_ blocks: [ShapeBlock], id: UUID, toTime: Double,
                          duration: Double) -> [ShapeBlock] {
        blocks.map { b in
    ...
    static func moveEnd(_ blocks: [ShapeBlock], id: UUID, toTime: Double,
                        duration: Double) -> [ShapeBlock] {
        blocks.map { b in
            guard b.id == id else { return b }
            var c = b
    ...
    static func moveBlock(_ blocks: [ShapeBlock], id: UUID, toBegin: Double,
                          duration: Double) -> [ShapeBlock] {
        blocks.map { b in
            guard b.id == id else { return b }
    ...
    static func remove(_ blocks: [ShapeBlock], id: UUID) -> [ShapeBlock] {
    ...
    static func bringForward(_ blocks: [ShapeBlock], id: UUID) -> [ShapeBlock] {
    ...
    static func sendBackward(_ blocks: [ShapeBlock], id: UUID) -> [ShapeBlock] {
    ...
    static func moveToFront(_ blocks: [ShapeBlock], id: UUID) -> [ShapeBlock] {
    ...
    static func moveToBack(_ blocks: [ShapeBlock], id: UUID) -> [ShapeBlock] {
        guard let i = blocks.firstIndex(where: { $0.id == id }) else { return blocks }
        var b = blocks
        let block = b.remove(at: i)
        b.insert(block, at: 0)
    ...
    static func subRows(_ blocks: [ShapeBlock]) -> [[ShapeBlock]] {
